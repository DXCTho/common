  ...
  stages {
   ...
   stage('Artifactory Repo') {
		   ...
		   withCredentials([string(credentialsId: 'JFrog_API_Key', variable: 'TOKEN')]) {
                sh '''
  			curl -H 'X-JFrog-Art-Api:'${TOKEN}'' -X PUT "http://172.17.0.3:8081/artifactory/libs-release-local/com/dai/xmp/${XMP_IMAGE}/${XMP_POM_VERSION}.jar" -T target/${XMP_IMAGE}-${XMP_POM_VERSION}.jar
